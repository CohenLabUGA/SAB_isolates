...
Next, I will create two objects called "path\_" and "path_out" with the location to reading data in and save output

```{r set up script, comment=''}
...
```{r clean data, comment=''}
physioData$culture <- str_replace_all(physioData$culture, c('4' = 'C. closterium UGA4','8' = 'C. closterium UGA8','6' = 'G. oceanica','13' = 'G. huxleyi'))
...
```{r pull out growth rates, comment=''}
colnames(physioData)
growth <- physioData %>% 
...
```{r make figure df, include=FALSE}
...
|   Now the data is ready to plot. I will create a bar plot with error bars showing the standard deviation for the growth rate data using ggplot. In class, we learned how to make ggplots using aes() to select the x and y axis, and using geom\_ to select the type of figure we wish to make.

```{r make plot, comment=''}
mu.plot <-{
  avg.mu$culture <- factor(avg.mu$culture, levels=c('G. oceanica','G. huxleyi','C. closterium UGA8','C. closterium UGA4'))
  brp <- ggplot(avg.mu, aes(culture, mu, fill=treatment))+
    ...
    geom_errorbar(aes(ymin=mu-mu.sd, ymax=mu+mu.sd),position = position_dodge(width=0.9), width=0.2, linewidth=1) +
    ...
    theme_pubr()+
    theme(text = element_text(size=10),axis.title.x=element_blank(), axis.text.x = element_text(face="italic"),legend.position = 'bottom', strip.placement = 'outside',strip.text = element_text( face='bold'),strip.background = element_rect(fill='white', color='white'))  +
